...
export class AntibioticoComponent extends GeneralComponent {
  ...
  fields: FormlyFieldConfig[] = [
    {
      ...
      fieldGroup: [
        ...
        },
        {
          ...
          hooks: {
            onInit: field => {
              const campo = field.form.get('numPacTemperatura');
              if (campo != null) {
                campo.valueChanges.subscribe(x => {
                  if (this.parametrosVitales != undefined) {
  ...
  ngOnInit() {
    ...
    // console.log("entro a AntibioticoComponent", this.detalle);
    // this.edadPaciente = this.detalle.header.dos[0].informacion[0].texto;


    //  console.log("parametros ****", this.parametrosVitales);
    if (this.detalle) {
      ...
      this.getDetalleAntibiotico(this.detalle);
  ...
  getDetalleAntibiotico(detalle: DetalleAplicacionMezcla) {
    this._seguimientoService.getDetalleAntibiotico(detalle.registro.idMezclaAplicDiaDosis).then(
      resp => {
        this.detalleAntibiotico = resp;
        //console.log("detalle atb: ", resp);
  ...
  public registrarSignos() {
    ...
    // console.log("registrarSignos", this.model);
    //  console.log("registrarSignos", this.form);
    //  console.log("registrarSignos", this.form.controls['numPacTemperatura']);
    //  console.log("registrarSignos", this.form.controls['numPacTemperatura'].errors['pattern']);
    ...
    switch (blnInicio) {
      case true:
        ...
        if (this.model.stpAplicacInicio != null) {
          ...
          console.log("horcalendarioa: ", calendario);
  ...
  public btnFinalizarAplicacion() {
    console.log("btnFinalizar ", this.blnBloquearFinalizar);
  ...
  private mostrarDialogo(titulo: string, mensaje: string, cancelar: string, btnOk: string) {
    return this._dialog.open(DialogComponent,
      this._dialogService.modalGenerico(titulo, mensaje, cancelar, btnOk)
