...
@Component({
  ...
  template: `
    <div style="display: flex; flex-direction: column; margin-top: 4px;" [class.has-error]="showError">
      <label *ngIf="to.label &&  to['hideLabel'] !== true" [attr.for]="id" class="form-label">
        {{ to.label }}<span *ngIf="to.required && to['hideRequiredMarker']  !== true" aria-hidden="true">*</span>
      </label>
